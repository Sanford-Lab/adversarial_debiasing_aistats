# Run alpha_sensitivity.R n_replicates times per debias weight; report mean and 95% CI
# for the adv_tuned estimate (same lambda construction as process_server_data.R).
#
# Child Rscript processes inherit your shell env; export RETICULATE_PYTHON (and optionally
# ALPHA_SENSITIVITY_FAST) before running. Do not rely on passing full Sys.getenv() via system2.
...
debias_weights <- {
  ...
  if (nzchar(w_env)) {
    as.numeric(strsplit(w_env, ",", fixed = TRUE)[[1]])
  } else {
    c(0.5, 1, 2, 5)
  }
}
...
alpha_script <- file.path(code_dir, "alpha_sensitivity.R")
...
run_weight_batch <- function(w, n_rep, script_path, rscript) {
  ...
  est_tree <- numeric(n_rep)
  est_adv <- numeric(n_rep)
  mse_measurement <- numeric(n_rep)

  ## Do not pass Sys.getenv() into system2(env=): large / odd values break the shell.
  ## Export RETICULATE_PYTHON, ALPHA_SENSITIVITY_FAST, etc. before starting this script.
  for (i in seq_len(n_rep)) {
    ...
    if (!file.exists(tmp_files[[i]])) {
      stop(
        "alpha_sensitivity.R did not write output for weight ", w, " rep ", i,
    ...
    est_tree[[i]] <- r$estimate_tree
    est_adv[[i]] <- r$estimate_adv_corr
    mse_measurement[[i]] <- r$mse_measurement_model
  }

  lam <- adv_tuned_lambda(est_tree, est_adv)
  tuned <- (1 - lam) * est_tree + lam * est_adv
...
main <- function() {
  ...
  out_dir <- Sys.getenv(
    ...
    unset = file.path(dirname(code_dir), "data", "alpha_sensitivity_results")
  )
