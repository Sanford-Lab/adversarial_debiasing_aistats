...
out_dir <- Sys.getenv(
  ...
  unset = file.path("data", "alpha_sensitivity_results")
)
in_csv <- file.path(out_dir, "summary_by_weight.csv")
out_tex <- file.path(out_dir, "summary_by_weight_table.tex")

if (!file.exists(in_csv)) {
  stop("Missing input file: ", in_csv)
}

dat <- read_csv(in_csv, show_col_types = FALSE)

if (!all(c("debias_wt", "n") %in% names(dat))) {
  stop("Input CSV does not have required columns debias_wt and n.")
}
...
dat_fmt <- dat %>%
  mutate(
    ...
    ci_est = ifelse(
      ...
      "--",
      paste0("[", fmt(ci_lower, 4), ", ", fmt(ci_upper, 4), "]")
    ),
    mse_mean = fmt(mse_mean, 4),
    ci_mse = ifelse(
      is.na(mse_ci_lower) | is.na(mse_ci_upper),
      "--",
      paste0("[", fmt(mse_ci_lower, 4), ", ", fmt(mse_ci_upper, 4), "]")
    ),
    n = as.integer(n)
  ) %>%
...
caption_txt <- paste0(
  "Alpha-sensitivity results for the tuned adversarial measurement model (yhat\\_adv\\_corr) on simulation data ",
...
header <- c(
  ...
  "\\label{tab:alpha_sensitivity_results}",
