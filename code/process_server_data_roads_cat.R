...
files <- list.files(path = "../data/progressive_sampling_roads_cat/", pattern = "\\.RDS$", full.names = TRUE)
...
lambda_by_size <- results_df %>%
  ...
  summarize(
    lambda = cov(
      estimate[model == "forest"], 
      estimate[model == "forest"] - estimate[model == "yhat_adv_corr"]
    ) / var(
      estimate[model == "forest"] - estimate[model == "yhat_adv_corr"]
    )
  ) %>%
...
ground_truth_data <- results_df %>%
  ...
  rename(forest_estimate = estimate)
...
tuned_data <- ground_truth_data %>%
  ...
  mutate(
    adv_tuned_estimate = (1 - lambda) * forest_estimate + 
...
ground_truth_mean <- results_df %>% 
  filter(sample_size == 10000, model == "ground_truth") %>%
...
(results_df %>% 
  ...
  geom_vline(xintercept = ground_truth_mean, aes(color = "Ground Truth Sample"), linetype = "dotted") +
...
ggsave(filename = "figures/baseline_vs_adversarial_progressive_all_after10k_roads_cat.png", width = 12, height = 6)
...
model_ci_plot <- ggplot(summary_df, aes(x = mean_estimate, y = model)) +
  geom_vline(xintercept = ground_truth_mean, color = "red", linetype = "dotted", linewidth = 0.8) +
  geom_point(size = 3) +
...
ggsave(filename = "../figures/model_estimates_samplesize_ci_roads_cat.png", width = 12, height = 12)
...
ggsave(
  filename = "../figures/compare_adversaries_plot_roads_cat.png",
  plot = compare_adversaries_plot,
  width = 10,
  height = 7,
  dpi = 300
