...
df <- read.csv("../data/processed/w_africa_l7_model_prepped.csv")

# ------------------------- DATA PREP -----------------------------------------

df <- df[!is.na(df$distance), ]  # Remove rows with NA covariates.
sapply(df, function(a){ sum(is.na(a)) })

# Remove top-coded distance-to-road observations
df <- df[df$distance < 32000, ]
df$distance = df$distance/1000  # change to KM for interpretability

# Predict binary forest / not-forest, correct in model for class imbalance.
df$forest <- if_else(df$land_use == "forest", 1, 0)
table(df$forest)

# Log of distance to road is treatment variable.
df$D = log1p(df$distance)

   print("read data")
ssize <- nrow(df)
new_df <- df %>% sample_n(size = ssize, replace = FALSE) %>% as_tibble()
rm(df)
...
results_df <- foreach(size = sizes, .packages = c("keras3", "tensorflow", "boot", "tidyverse", "doParallel", "mice", "PTDBoot", "ipd"), .combine = 'rbind') %dopar% {
  # Ensure Keras/TensorFlow environment is active in the worker
  library(keras3)
  library(tensorflow)
  ...
  for (run in 1:num_runs) {
    ...
    sample_data$yhat_lr <- train_all_folds(x = sample_data[, c("fold", "forest", "D", train_vars)],
                                          y_name =  "forest", treat_name = "D", y_class = "categorical",
                                          debias_wt = 0, epochs = 50, model = "deep_nn", verbose = F)



  sample_data <- sample_data %>% mutate(nu = forest - yhat_lr)
  ...
  print(summary(lm(forest ~ D, data = sample_data)))
  ...
  sample_data$yhat_lr <- unlist(rev(adb_model(x = sample_data[, c("fold", "forest", "D", train_vars)],
                                        y_name =  "forest", treat_name = "D", y_class = "categorical",
                                        debias_wt = 0, epochs = 50, model = "deep_nn",
                                        f = 1, predicttrain = TRUE, verbose = F)))

  sample_data$yhat_adv <- c(sample_data$forest[1:size],adb_model(x = sample_data[, c("fold", "forest", "D", train_vars)],
                                          y_name =  "forest", treat_name = "D", y_class = "categorical",
                                          debias_wt = 20, epochs = 100, model = "deep_nn",
                                          f = 1, predicttrain = FALSE, verbose = F))

  sample_data$yhat_adv_corr <- c(sample_data$forest[1:size],adb_model(x = sample_data[, c("fold", "forest", "D", train_vars)],
                                              y_name =  "forest", treat_name = "D",y_class = "categorical",
                                              debias_wt = 1, debias_method = "corr", epochs = 100, model = "deep_nn",
                                              f = 1, predicttrain = FALSE, verbose = F))

  # Add the mg debiaser method
  sample_data$yhat_adv_mg <- c(sample_data$forest[1:size], adb_model(
  ...
  models_to_run <- c(
      "forest", "yhat_lr", "yhat_adv", "yhat_adv_corr", "yhat_adv_mg",
    ...
    for (model_name in models_to_run) {
      ...
      if (model_name == "forest") {
        model_summary <- summary(lm(forest ~ D, data = GoodDat_samp)) # Use labeled data
        ...
        imputation_df$forest[imputation_df$fold == 1] <- NA # Set forest to NA for "unlabeled" part
        imputation_df_for_mice <- imputation_df %>% dplyr::select(forest, D, yhat_lr) # yhat_lr can be auxiliary
        ...
        current_rsq <- summary(lm(forest ~ D, data = completed_data_for_rsq))$r.squared
        ...
        PTD_PredictionDat_completeSamp <- GoodDat_samp %>% dplyr::mutate(forest = yhat_lr)
        PTD_PredictionDat_incompleteSamp <- UnlabeledDat_samp %>%
          dplyr::select(-forest) %>% # Remove original forest from unlabeled part
          dplyr::rename(forest = yhat_lr) # Use yhat_lr as the outcome proxy
        ...
        current_rsq <- summary(lm(as.formula(paste0(model_name, " ~ forest")), data = sample_data))$r.squared
...
write_rds(results_df, file = paste0("../data/progressive_sampling_roads_cat/", output_file))
